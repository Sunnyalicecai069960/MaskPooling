...
class TestSet(Dataset):
  ...
  def eval(
    ...
    with measure_time('Computing distance...', verbose=verbose):
      ...
      #print(feat[q_inds].shape)
      #print(feat[g_inds].shape)
      q_g_dist = compute_dist(np.squeeze(feat[q_inds]), np.squeeze(feat[g_inds]), type='euclidean')
    ...
    if any(mq_inds):
      ...
      with measure_time('Multi Query, Computing distance...', verbose=verbose):
        ...
        mq_g_dist = compute_dist(np.squeeze(mq_feat), np.squeeze(feat[g_inds]), type='euclidean')
    ...
    if to_re_rank:
      ...
      with measure_time('Re-ranking distance...', verbose=verbose):
        ...
        q_q_dist = compute_dist(np.squeeze(feat[q_inds]), np.squeeze(feat[q_inds]), type='euclidean')
        # gallery-gallery distance
        g_g_dist = compute_dist(np.squeeze(feat[g_inds]), np.squeeze(feat[g_inds]), type='euclidean')
      ...
      if any(mq_inds):
        with measure_time('Multi Query, Re-ranking distance...',
          ...
          mq_mq_dist = compute_dist(np.squeeze(mq_feat), np.squeeze(mq_feat), type='euclidean')
